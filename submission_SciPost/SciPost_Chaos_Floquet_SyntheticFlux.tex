...
Floquet--Lyapunov consistency and matched sensing bounds for flux-controlled hyperchaos in dissipative optomechanics\\
}}\end{center}
...
\section*{Abstract}
{\bf
Driven dissipative optomechanical networks combine radiation-pressure
nonlinearity, coherent phonon transport, and synthetic gauge phases in one
tunable platform. We show that the gauge-invariant flux phase controls the
order of hyperchaos in such a network. On a six-dimensional semiclassical
model with one optical and two mechanical modes, a phase-consistent
Floquet--Lyapunov protocol---monodromy multipliers, the full Lyapunov spectrum,
and the dissipative volume balance certifying one another on the same
orbit---resolves a drive- and coupling-gated transition from stability to chaos
and then hyperchaos through a Neimark--Sacker torus bifurcation, the flux phase
tuning the number of simultaneously unstable directions up to four. The
measured cavity quadrature remains \num{7.6}--\num{17} times above the noise floor
in the chaotic sector, and a matched measurement-Fisher calculation bounds the
force-sensing gain to at most \SI{32}{\percent} over a flux-off reference and
\SI{16}{\percent} over a single-mode reference (a typical median gain of a few percent), with no quantum or SQL-beating claim.
Both results are model-level demonstrations, the hyperchaos regime lying
\num{2542}$\times$ beyond anchored optomechanical couplings.
}
...
class \cite{muthukumar2025doi}. Together these three strands constitute a novel
synergetic link between synthetic-gauge physics, nonlinear dynamics, and
quantum-limited measurement: each ingredient is individually standard, but their
joint certification on a single orbit is the methodological advance, and the
consistency protocol transfers directly to any driven dissipative system. The
central comparison is between a
...
(\num{1e-4} for the weak-coupling map, \num{1e-3} for the strong-coupling map and its checks); the Kaplan--Yorke dimension instead sums strictly positive exponents,
...
article is bound to the corresponding machine-readable artifact in that
repository.
